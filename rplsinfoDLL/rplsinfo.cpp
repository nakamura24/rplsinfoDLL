...
int rplsinfo(LPCWSTR filename, ProgInfo *proginfo)
{
	...
	_wfullpath(proginfo->fullpath, filename, _MAX_PATH);														// フルパス名取得
	_wsplitpath_s(proginfo->fullpath, NULL, 0, NULL, 0, proginfo->fname, _MAX_PATH, proginfo->fext, _MAX_PATH);			// ベースファイル名と拡張子

	if(sfiletype == FILE_RPLS) {
		readRplsProgInfo(hReadFile, proginfo, false);
	} else 	{
		BOOL bResult = readTsProgInfo(hReadFile, proginfo, sfiletype, DEFAULTTSFILEPOS, false);
		if(!bResult) {
			wprintf(L"TSファイル %s から有効な番組情報を検出できませんでした.\n", filename);
			CloseHandle(hReadFile);
			return(3);
